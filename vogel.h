#define MAX 10000
float findTwoLowestValueInRow(int m, int n, float c[MAX][MAX]) {
    float a[MAX];
    for (int i = 0; i < m; i++) {a[i] = c[n][i];}
    int i,I = -1;
    float m1 = 1e9, m2 = 1e9;
    for (i = 0; i < m; i++) {
...
float findTwoLowestValueInCol(int m, int n, float c[MAX][MAX]) {
    float a[MAX];
    for (int j = 0; j < n; j++) {a[j] = c[j][m];}
    int j,J = -1;
    float m1 = 1e9, m2 = 1e9;
    for (j = 0; j < n; j++) {
...
void vogelMethod(int m, int a[MAX], int n, int b[MAX], float c[MAX][MAX], int x[MAX][MAX], float *totalCost) {
    ...
    while (1) {
        ...
        for (int j = 0; j < m; j++) {
			penCol[j] = colDoneYet[j]? -1e9 :findTwoLowestValueInCol(j, n, c);
		}
